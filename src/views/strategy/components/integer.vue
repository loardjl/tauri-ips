...
const onConfirm = ({ selectedOptions }) => {
  ...
  inputValue.value = Number(
    selectedOptions[0].value + selectedOptions[1].value + selectedOptions[2].value
  )
}
function padNumber(num, n = 3) {
  const str = num.toString()
  const padLength = n - str.length
  const strlen = '0'.repeat(padLength) + str
  const arr = strlen.split('').map(item => item)
  integerNumArr.value = arr
}
padNumber(inputValue.value)
</script>
